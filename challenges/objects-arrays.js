...
const tyrannosaurus = {
  ...
  rawr: () => {return 'RAWERSRARARWERSARARARRRR!'}
};
...
const contactInfo = graduates.map(graduate => {return `${graduate.first_name} ${graduate.email}`});
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.map(graduate => {if (graduate.university.includes('Uni')) return graduate.university}).filter(item => item !== undefined);
console.log(uni);
...
zooAnimals.filter(animal => {animal.population < 5 && lowerPopulation.push(animal)});
...
*/
let populationTotal = zooAnimals.map(animal => animal.population).reduce((acc, value) => {return acc + value}); // returns 1280??
